...
{
    [Route("[controller]")]
    public class BaseFundingController<T> : BaseController where T : IBaseEntity
    {
        ...
        protected IFundingBaseEntityViewService<T> _contentViewService;
        ...
        public BaseFundingController(
            ...
            IFundingBaseEntityViewService<T> contentViewService) : base(contentService)
        {
        ...
        {
            var user = await _userService.FindByClaimAsync(this.User);
            if (user == null) return RedirectToAction("Index", "Funding");
