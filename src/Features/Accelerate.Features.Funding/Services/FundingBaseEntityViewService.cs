...
{
    public class FundingBaseEntityViewService<T> : IFundingBaseEntityViewService<T> where T : IBaseEntity
    {
        ...
        public FundingBaseEntityViewService(
        ...
        private async Task<FundingBasePage<T>> CreateBaseFundingPage(UsersUser user)
        {
            var baseModel = await _portalContentService.CreateAuthenticatedContent(user);
            var viewModel = new FundingBasePage<T>(baseModel);
            return viewModel;
        }
        public async Task<FundingNotFoundPage> CreateNotFoundPage(UsersUser user, string title = null, string description = null)
        {
            var model = await CreateBaseFundingPage(user);
            var viewModel = new FundingNotFoundPage(model);
        ...
        public virtual async Task<FundingBasePage<T>> CreateListPage(UsersUser user, IEnumerable<T> items)
        {
            var viewModel = await CreateBaseFundingPage(user);
        ...
        public virtual async Task<FundingBasePage<T>> CreateIndexPage(UsersUser user)
        {
            var viewModel = await CreateBaseFundingPage(user);
            //var viewModel = new AdminCreatePage(model); 
            var pageName = "All";
        ...
        public virtual async Task<FundingBasePage<T>> CreateEntityPage(UsersUser user, T item)
        {
            var viewModel = await CreateBaseFundingPage(user);
        ...
        public virtual async Task<FundingFormPage> CreateNewPage(UsersUser user)
        {
            var model = await CreateBaseFundingPage(user);
            var viewModel = new FundingFormPage(model);
        ...
        public virtual async Task<FundingFormPage> CreateUpdatePage(UsersUser user, T item)
        {
            var model = await CreateBaseFundingPage(user);
            var viewModel = new FundingFormPage(model);
            //var viewModel = new AdminIndexPage<T>(model);
        ...
        public virtual AjaxForm CreateEntityForm(UsersUser user, T? item, PostbackType type = PostbackType.POST)
        {
            ...
            {
                Action = item == null
                    ? $"{Foundations.Common.Constants.ApiPaths.VersionPath}/Funding/{this.EntityName.ToLower()}"
                    : $"{Foundations.Common.Constants.ApiPaths.VersionPath}/Funding/{this.EntityName.ToLower()}/{item.Id}",
                Type = type,
        ...
        { 
            return new ButtonGroup()
            {
                ...
                Items = new List<ButtonItem>()
                {
                    ...
                    {
                        Text = $"Edit",
                        Href = $"/Funding/{entity}/{item.Id}/Edit",
                        Class = "relative flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-200 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
                        Icon = "edit"
                    },
                    new ButtonItem()
                    ...
                    {
                        Text = $"Search",
                        Href = $"/Funding/{entity}/{item.Id}/Search",
                        Class = "relative flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-200 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        ...
        public AjaxForm CreateFormDelete(UsersUser user, T item)
        {
            var model = new AjaxForm()
            {
                Action = $"/api/Funding{this.EntityName.ToLower()}/{item.Id}",
        ...
        public NavigationItem? GetLink(T x)
        {
            ...
            return new NavigationItem()
            {
                ...
                Href = $"/Funding/{this.EntityName}s/{x.Id}",
            };
        }
    }
}
