...
{
    [Route("[controller]")]
    public class BaseRatesController<T> : BaseController where T : IBaseEntity
    {
        ...
        public BaseRatesController(
            ...
            IRatesBaseEntityViewService<T> contentViewService) : base(contentService)
        {
        ...
        {
            var user = await _userService.FindByClaimAsync(this.User);
            if (user == null) return RedirectToAction("Index", "Rates");
