...
{
    public class RatesBaseEntityViewService<T> : IRatesBaseEntityViewService<T> where T : IBaseEntity
    {
        ...
        public RatesBaseEntityViewService(
        ...
        private async Task<RatesBasePage<T>> CreateBaseRatesPage(UsersUser user)
        {
            var baseModel = await _portalContentService.CreateAuthenticatedContent(user);
            var viewModel = new RatesBasePage<T>(baseModel);
            return viewModel;
        }
        public async Task<RatesNotFoundPage> CreateNotFoundPage(UsersUser user, string title = null, string description = null)
        {
            var model = await CreateBaseRatesPage(user);
            var viewModel = new RatesNotFoundPage(model);
        ...
        public virtual async Task<RatesBasePage<T>> CreateListPage(UsersUser user, IEnumerable<T> items)
        {
            var viewModel = await CreateBaseRatesPage(user);
        ...
        public virtual async Task<RatesBasePage<T>> CreateIndexPage(UsersUser user)
        {
            var viewModel = await CreateBaseRatesPage(user);
            //var viewModel = new AdminCreatePage(model); 
            var pageName = "All";
        ...
        public virtual async Task<RatesBasePage<T>> CreateEntityPage(UsersUser user, T item)
        {
            var viewModel = await CreateBaseRatesPage(user);
        ...
        public virtual async Task<RatesFormPage> CreateNewPage(UsersUser user)
        {
            var model = await CreateBaseRatesPage(user);
            var viewModel = new RatesFormPage(model);
        ...
        public virtual async Task<RatesFormPage> CreateUpdatePage(UsersUser user, T item)
        {
            var model = await CreateBaseRatesPage(user);
            var viewModel = new RatesFormPage(model);
            //var viewModel = new AdminIndexPage<T>(model);
        ...
        public virtual AjaxForm CreateEntityForm(UsersUser user, T? item, PostbackType type = PostbackType.POST)
        {
            ...
            {
                Action = item == null
                    ? $"{Foundations.Common.Constants.ApiPaths.VersionPath}/Rates/{this.EntityName.ToLower()}"
                    : $"{Foundations.Common.Constants.ApiPaths.VersionPath}/Rates/{this.EntityName.ToLower()}/{item.Id}",
                Type = type,
        ...
        { 
            return new ButtonGroup()
            {
                ...
                Items = new List<ButtonItem>()
                {
                    ...
                    {
                        Text = $"Edit",
                        Href = $"/Rates/{entity}/{item.Id}/Edit",
                        Class = "relative flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-200 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
                        Icon = "edit"
                    },
                    new ButtonItem()
                    ...
                    {
                        Text = $"Search",
                        Href = $"/Rates/{entity}/{item.Id}/Search",
                        Class = "relative flex cursor-pointer select-none items-center rounded px-2 py-1.5 text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-200 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
        ...
        public AjaxForm CreateFormDelete(UsersUser user, T item)
        {
            var model = new AjaxForm()
            {
                Action = $"/api/Rates{this.EntityName.ToLower()}/{item.Id}",
        ...
        public NavigationItem? GetLink(T x)
        {
            ...
            return new NavigationItem()
            {
                ...
                Href = $"/Rates/{this.EntityName}s/{x.Id}",
            };
        }
    }
}
