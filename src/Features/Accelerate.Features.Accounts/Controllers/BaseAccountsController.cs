...
{
    [Route("[controller]")]
    public class BaseAccountsController<T> : BaseController where T : IBaseEntity
    {
        ...
        protected IAccountsBaseEntityViewService<T> _contentViewService;
        ...
        public BaseAccountsController(
            ...
            IAccountsBaseEntityViewService<T> contentViewService) : base(contentService)
        {
        ...
        {
            var user = await _userService.FindByClaimAsync(this.User);
            if (user == null) return RedirectToAction("Index", "Accounts");
